    ...
    * Inner-loop should not call any functions or write non-local variables
    ...
    def _reduce(
        ...
        for i in prange(len(out)):
            ...
            # Accumulate reduce over this temp value
            temp = out[out_pos]
...
def _tensor_matrix_multiply(
    ...
    * Inner loop should have no global writes, 1 multiply.
    ...
    for n in prange(max(a_shape[0], b_shape[0])):
        # Adjust for batch stride
        a_batch_offset = n * a_batch_stride
        b_batch_offset = n * b_batch_stride
        ...
        for i in range(a_shape[-2]):
            for j in range(b_shape[-1]):
                ...
                for k in range(a_shape[-1]):
                    # Convert 2D to 1D storage coordinates
                ...
                # Store the result
                out_idx = n * out_strides[0] + i * out_strides[-2] + j * out_strides[-1]
                out[out_idx] = sum
